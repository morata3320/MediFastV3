...
export function AuthPage({ mode = "login", loginUser, registerUser, submitting, onSuccess }) {
  ...
  return <main className="container auth-page"><section className="modal auth-card"><h2>{isLogin ? "Iniciar sesión" : "Crear cuenta"}</h2><p className="hint">Acceso protegido con JWT y roles. Las credenciales no se muestran en pantalla.</p><form onSubmit={submit}>{isLogin ? <div className="field"><label>Usuario o correo</label><input required autoComplete="username" value={form.identifier} onChange={(event) => setForm({ ...form, identifier: event.target.value })} /></div> : <><div className="field"><label>Usuario</label><input required autoComplete="username" value={form.username} onChange={(event) => setForm({ ...form, username: event.target.value })} /></div><div className="field"><label>Correo</label><input required autoComplete="email" type="email" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} /></div></>}<div className="field"><label>Contraseña</label><input required autoComplete={isLogin ? "current-password" : "new-password"} type="password" value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value })} /></div><button className="btn-full" disabled={submitting} type="submit">{submitting ? "Procesando..." : isLogin ? "Entrar" : "Registrarme"}</button></form><p className="hint">{isLogin ? <Link to="/login?mode=register">Crear una cuenta</Link> : <Link to="/login">Ya tengo cuenta</Link>}</p></section></main>;
}
